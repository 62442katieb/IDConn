...
df = pd.DataFrame(index=index, columns=['assortativity', 'transitivity'], dtype=np.float64)
...
for subject in subjects:
    for session in sessions:
        ...
        try:
            ...
            shen_corrmat = np.genfromtxt(join(sink_dir, '{0}-session-{1}-rest_network_corrmat_shen2015.csv'.format(subject, session)), delimiter=",")
            ...
            craddock_corrmat = np.genfromtxt(join(sink_dir, '{0}-session-{1}-rest_network_corrmat_craddock2012.csv'.format(subject, session)), delimiter=",")
            ...
            for p in np.arange(kappa_upper, kappa_lower, 0.01):
                ...
                #global efficiency
                ge = bct.assortativity_wei(shen_thresh)
                ge_s.append(ge)
                ge = bct.assortativity_wei(craddock_thresh)
                ge_c.append(ge)

                #modularity
                md = bct.transitivity_wu(shen_thresh)
                md_s.append(md)
                md = bct.transitivity_wu(craddock_thresh)
                md_c.append(md)
            df.at[(subject, session, 'rest', 'rest', 'shen2015'), 'assortativity'] = np.trapz(ge_s, dx=0.1)
            df.at[(subject, session, 'rest', 'rest', 'shen2015'), 'transitivity'] = np.trapz(md_s, dx=0.1)
            df.at[(subject, session, 'rest', 'rest', 'craddock2012'), 'assortativity'] = np.trapz(ge_c, dx=0.1)
            df.at[(subject, session, 'rest', 'rest', 'craddock2012'), 'transitivity'] = np.trapz(md_c, dx=0.1)
