...
df = pd.DataFrame(
    {
        "global efficiency phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc default mode-left central executive phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc default mode-right central executive phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc left central executive-right central executive phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc hippo-default mode phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc hippo-left central executive phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc hippo-right central executive phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "le default mode phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "le right central executive phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "le left central executive phy": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "global efficiency gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc default mode-left central executive gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc default mode-right central executive gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc left central executive-right central executive gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc hippo-default mode gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc hippo-left central executive gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "fc hippo-right central executive gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "le default mode gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "le right central executive gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
        ),
        "le left central executive gen": np.empty(
            [
                len(subjects),
            ],
            dtype=float,
...
for subject in subjects:
    ...
    for run in runs:
        ...
        ntwk_masker = NiftiLabelsMasker(
            icn, standardize=True, high_pass=highpass, t_r=2.0, verbose=1
        )
    ...
    for condition in conditions:
        ...
        np.savetxt(
            join(data_dir, subject, "{0}-{1}-corrmat.csv".format(subject, condition)),
            corrmats[condition],
        )
        df.at[
            subject, "fc default mode-right central executive {0}".format(condition)
        ] = corrmats[condition][12, 14]
        df.at[
        ...
        for network in networks:
            df.at[subject, "le {0} {1}".format(labels[network], condition)] = np.trapz(
                loceff[labels[network]], dx=0.1
            )
            ...
            df.at[
                subject, "fc hippo-{0} {1}".format(labels[network], condition)
            ] = pearsonr(hipp[condition], network_ts)[0]
...
for subject in subjects:
    phy += np.genfromtxt(
        join(data_dir, subject, "{0}-phy-corrmat.csv".format(subject)), delimiter=" "
    )
    gen += np.genfromtxt(
        join(data_dir, subject, "{0}-gen-corrmat.csv".format(subject)), delimiter=" "
    )
