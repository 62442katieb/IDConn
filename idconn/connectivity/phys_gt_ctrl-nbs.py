...
tasks = {"retr": ["Physics", "General"], "fci": ["Physics", "NonPhysics"]}
for task in tasks.keys():
    for group in subject_groups.keys():
        ...
        if group == "modeling":
            ...
            adjacency.to_csv(
                join(data_dir, "nbs", "{0}_students-{1}.csv".format(group, task))
            )

            fig.savefig(
                join(fig_dir, "nbs-{0}_students-{1}.png".format(group, task)), dpi=300
            )
