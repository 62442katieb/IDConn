...
df = pd.DataFrame(
    columns=["assortativity", "transitivity", "efficiency", "charpath", "modularity"],
    index=index,
    dtype=np.float64,
)
            ...
            for i in np.arange(0, len(tasks[task][0]["conditions"])):
                ...
                for mask in masks:
                    try:
                        ...
                        lab_notebook.at[
                            (subject, session, task, conds[i], mask), "end"
                        ] = str(datetime.datetime.now())
                    except Exception as e:
                        print(e, subject, session)
                        lab_notebook.at[
                            (subject, session, task, conds[i], mask), "errors"
                        ] = [e, str(datetime.datetime.now())]
                    df.to_csv(
                        join(
                            sink_dir,
                            "physics-learning-tasks_graphtheory_shen+craddock_last.csv",
                        ),
                        sep=",",
                    )
...
df.to_csv(
    join(
        sink_dir,
        "physics-learning-tasks_graphtheory_shen+craddock_last_{0}.csv".format(today),
    ),
    sep=",",
)
lab_notebook.to_csv(
    join(sink_dir, "LOGphysics-learning-tasks-graphtheory_{0}.csv".format(today))
)
