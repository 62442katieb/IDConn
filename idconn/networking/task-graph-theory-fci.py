...
df = pd.DataFrame(
    columns=["efficiency", "charpath", "modularity"], index=index, dtype=np.float64
)
            ...
            for i in np.arange(0, len(tasks[task][0]["conditions"])):
                ...
                for mask in masks:
                    ...
                    df.to_csv(
                        join(
                            sink_dir,
                            "physics-learning-tasks_graphtheory_shen+craddock.csv",
                        ),
                        sep=",",
                    )
...
df.to_csv(
    join(
        sink_dir,
        "physics-learning-fci_graphtheory_shen+craddock.csv_{0}.csv".format(
...
lab_notebook.to_csv(
    join(
        lab_notebook_dir,
        "physics-learning-fci-graphtheory_{0}.csv".format(str(datetime.datetime.now())),
    )
)
