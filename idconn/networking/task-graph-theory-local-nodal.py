                ...
                for mask in masks:
                    try:
                        ...
                        for j in np.arange(0, 268):
                            df.at[
                                (subject, session, task, conds[i], mask),
                                "lEff{0}".format(j),
                            ] = leff[j]
                            df.at[
                                (subject, session, task, conds[i], mask),
                                "clustCoeff{0}".format(j),
                            ] = ccoef[j]
                        lab_notebook.at[
                    ...
                    df.to_csv(
                        join(
                            sink_dir,
                            "physics-learning-tasks_graphtheory_shen+craddock_nodal.csv",
                        ),
                        sep=",",
                    )
...
df.to_csv(
    join(
        sink_dir,
        "physics-learning-tasks_graphtheory_shen+craddock_nodal{0}.csv".format(today),
    ),
    sep=",",
)
lab_notebook.to_csv(
    join(sink_dir, "LOG-physics-learning-tasks-graphtheory_nodal_{0}.csv".format(today))
)
