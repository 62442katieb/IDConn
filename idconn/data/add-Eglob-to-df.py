...
b_df = pd.read_csv(
    join(data_dir, "rescored", "physics_learning-nonbrain_OLS+fd-BayesianImpute.csv"),
    index_col=0,
    header=0,
)

gt_df = pd.read_csv(
    join(sink_dir, "physics-learning-tasks_graphtheory_shen+craddock.csv"),
    index_col=[0, 1, 2, 3, 4],
    header=0,
)

gt_df.drop(
    ["assortativity", "transitivity", "charpath", "modularity"], axis=1, inplace=True
)
...
big_df.to_csv(
    join(data_dir, "rescored", "physics_learning-nonbrain_OLS+fd+eff-missing.csv")
)

impute_pls = IterativeImputer(
    max_iter=10000, skip_complete=True, verbose=1, tol=1e-3, n_nearest_features=100
)
    ...
    for session in sessions:
        for task in tasks:
            for condition in conditions:
                ...
                big_df[new_conns] = (
                    big_df[new_conns]
                    / null_df.loc[sesh[session], task, cond, mask]["mean"]
                )
...
big_df.to_csv(
    join(
        data_dir, "rescored", "physics_learning-nonbrain_OLS+fd+eff-BayesianImpute.csv"
    )
)
