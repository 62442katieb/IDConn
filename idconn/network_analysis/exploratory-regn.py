...
f_gender_df = pd.read_csv(
    join(data_dir, "rescored_gender_identity_female-post.csv"), index_col=0, header=0
)
m_gender_df = pd.read_csv(
    join(data_dir, "rescored_gender_identity_male-post.csv"), index_col=0, header=0
)
phy_rt_df = pd.read_csv(
    join(data_dir, "retr_physcond_accuracy_by_gender_post.txt"), sep="\t", index_col=1
)
...
m_phy_p, m_phy_t, _ = permuted_ols(
    tested_vars=df_dudes[behav].values,
    target_vars=m_phy,
...
df_ladies.drop(321, axis=0, inplace=True)
df_ladies.drop(618, axis=0, inplace=True)
...
f_phy_p, f_phy_t, _ = permuted_ols(
    tested_vars=df_ladies[behav].values,
    target_vars=f_phy,
    model_intercept=True,
    verbose=1,
    n_perm=10000,
