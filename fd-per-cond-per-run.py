...
index = pd.MultiIndex.from_product([subjects, sessions, tasks.keys(), conds], names=['subject', 'session', 'task', 'condition'])
        ...
        for task in tasks.keys():
            ...
            try:
                for run in tasks[task][1]['runs']:
                    for condition in conditions:
                        ...
                        if task != 'reas':
                            ...
                            #print(timing['{0}-{1}'.format(run, condition)])
                    if task == 'fci':
                        fd = np.genfromtxt(join(preroc_dir, subject, 'session-{0}'.format(i), 'fci/fci-{0}/fci-{0}-ppi.feat'.format(run), '{0}-session-{1}-fd.txt'.format(subject, i)))
                    else:
                        fd = np.genfromtxt(join(data_dir, sessions[i], subject, '{0}-session-{1}_{2}-{3}_fd.txt'.format(subject, i, task, run)))
                    for condition in conditions:
                        print(fd[timing['{0}-{1}'.format(run, condition)][0,0].astype(int):(timing['{0}-{1}'.format(run, condition)][0,0]+timing['{0}-{1}'.format(run, condition)][0,1]+1).astype(int)].shape)
                        print(fd[timing['{0}-{1}'.format(run, condition)][1,0].astype(int):(timing['{0}-{1}'.format(run, condition)][1,0]+timing['{0}-{1}'.format(run, condition)][1,1]+1).astype(int)].shape)
                        print(fd[timing['{0}-{1}'.format(run, condition)][2,0].astype(int):(timing['{0}-{1}'.format(run, condition)][2,0]+timing['{0}-{1}'.format(run, condition)][2,1]+1).astype(int)].shape)
                        fd_cond['{0}-{1}'.format(run, condition)] = np.hstack((fd[timing['{0}-{1}'.format(run, condition)][0,0].astype(int):(timing['{0}-{1}'.format(run, condition)][0,0]+timing['{0}-{1}'.format(run, condition)][0,1]+1).astype(int)], fd[timing['{0}-{1}'.format(run, condition)][1,0].astype(int):(timing['{0}-{1}'.format(run, condition)][1,0]+timing['{0}-{1}'.format(run, condition)][1,1]+1).astype(int)], fd[timing['{0}-{1}'.format(run, condition)][2,0].astype(int):(timing['{0}-{1}'.format(run, condition)][2,0]+timing['{0}-{1}'.format(run, condition)][2,1]+1).astype(int)]))
                        print('cut fd for {0}, run {1}, {2}'.format(task, run, condition), fd_cond['{0}-{1}'.format(run, condition)].shape)
                ...
                for j in np.arange(0,len(conditions)):
                    ...
                    if task != 'fci':
                        sliced_fd[conditions[j]] = np.hstack((fd_cond['0-{0}'.format(conditions[j])], fd_cond['1-{0}'.format(conditions[j])]))
                        print(sliced_fd[conditions[j]].shape)
                    else:
                        sliced_fd[conditions[j]] = np.hstack((fd_cond['0-{0}'.format(conditions[j])], fd_cond['1-{0}'.format(conditions[j])], fd_cond['2-{0}'.format(conditions[j])]))
                        print(sliced_fd[conditions[j]].shape)
                for j in np.arange(0,len(conditions)):
                    df.loc[(subject, sessions[i], task, conds[j]), 'average fd'] = np.average(sliced_fd[conditions[j]])
            except Exception as e:
                print(subject, task, e)
df.to_csv(join(data_dir, 'avg-fd-per-condition-per-run_{0}.csv'.format(str(datetime.now()))))
