...
for subject in subjects:
    for i in np.arange(0,len(sessions)):
        try:
            df.loc[(subject, sessions[i]), 'average fd'] = np.genfromtxt(join(preproc_dir, subject, 'session-{0}/resting-state/resting-state-0/endor1.feat/mc/prefiltered_func_data_mcf_abs_mean.rms'.format(i)))
