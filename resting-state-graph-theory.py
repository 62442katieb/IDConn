...
index = pd.MultiIndex.from_product([subjects, sessions, tasks, condition, masks.keys()], names=['subject', 'session', 'task', 'condition', 'mask'])
df = pd.DataFrame(index=index, columns=['efficiency', 'charpath', 'modularity'], dtype=np.float64)

for subject in subjects:
    for session in sessions:
        lab_notebook.at[(subject, session),'start'] = str(datetime.datetime.now())
        if not exists(join(sink_dir, sesh[session], subject, '{0}-session-{1}_rest_mni-fnirt-epi-warp.nii.gz'.format(subject, session))):
            try:
                ...
                for mask in masks.keys():
                    ...
                    display = plot_roi(mask_nativespace, bg_img=join(data_dir, subject, 'session-{0}'.format(session), 'resting-state/resting-state-0/endor1.feat/reg/example_func.nii.gz'), colorbar=True)
        ...
        try:
            ...
            shen_corrmat = np.genfromtxt(join(sink_dir, sesh[session], subject, '{0}-session-{1}-rest_network_corrmat_shen2015.csv'.format(subject, session)), delimiter=",")
            ...
            craddock_corrmat = np.genfromtxt(join(sink_dir, sesh[session], subject, '{0}-session-{1}-rest_network_corrmat_craddock2012.csv'.format(subject, session)), delimiter=",")
            ...
            for p in np.arange(kappa_upper, kappa_lower, 0.01):
                ...
                md_c.append(md[1])
            df.at[(subject, session, 'rest', 'rest', 'shen2015'), 'efficiency'] = np.trapz(ge_s, dx=0.1)
            df.at[(subject, session, 'rest', 'rest', 'shen2015'), 'charpath'] = np.trapz(cp_s, dx=0.1)
            df.at[(subject, session, 'rest', 'rest', 'shen2015'), 'modularity'] = np.trapz(md_s, dx=0.1)
            df.at[(subject, session, 'rest', 'rest', 'craddock2012'), 'efficiency'] = np.trapz(ge_c, dx=0.1)
            df.at[(subject, session, 'rest', 'rest', 'craddock2012'), 'charpath'] = np.trapz(cp_c, dx=0.1)
            df.at[(subject, session, 'rest', 'rest', 'craddock2012'), 'modularity'] = np.trapz(md_c, dx=0.1)
...
df.to_csv(join(sink_dir, 'resting-state_graphtheory_shen+craddock_{0}.csv'.format(str(datetime.datetime.today()))), sep=',')
lab_notebook.to_csv(join(lab_notebook_dir, 'resting-state-graph-theory_{0}.csv'.format(str(datetime.datetime.now()))))
