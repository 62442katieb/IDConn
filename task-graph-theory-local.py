...
df = pd.DataFrame(columns=['assortativity', 'transitivity', 'efficiency', 'charpath', 'modularity'], index=index, dtype=np.float64)
            ...
            for i in np.arange(0, len(tasks[task][0]['conditions'])):
                ...
                for mask in masks:
                    try:
                        ...
                        corrmat = np.genfromtxt(join(sink_dir, '{0}-session-{1}_{2}-{3}_{4}-corrmat.csv'.format(subject, session, task, conditions[i], mask)), delimiter=' ')
                        ...
                        df.at[(subject, session, task, conds[i], mask), 'assortativity'] = np.trapz(at_s, dx=0.01)
                        df.at[(subject, session, task, conds[i], mask), 'transitivity'] = np.trapz(tr_s, dx=0.01)
                    ...
                    df.to_csv(join(sink_dir, 'physics-learning-tasks_graphtheory_shen+craddock_last.csv'), sep=',')

df.to_csv(join(sink_dir, 'physics-learning-tasks_graphtheory_shen+craddock_last_{0}.csv'.format(today)), sep=',')
lab_notebook.to_csv(join(sink_dir, 'LOGphysics-learning-tasks-graphtheory_{0}.csv'.format(today)))
