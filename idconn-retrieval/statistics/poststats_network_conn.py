...
for key in sig_f.keys():
    ...
    for region1 in regions:
        for region2 in regions:
            try:
                sig_prewmi_fci_p.at[region1,region2] = triplenetwork_sig.loc['p']['{0}-{1}'.format(region1, region2)]
                sig_prewmi_fci_t.at[region1,region2] = triplenetwork_sig.loc['t']['{0}-{1}'.format(region1, region2)]
    ...
    q.savefig('/Users/Katie/Dropbox/Projects/physics-retrieval/figures/f-{0}-{1}_conn_sig.png'.format(task, iq), dpi=300)

# +
sig_f = {'WMI': ['pre phys fci', fci_shen_pre_phys],
...
for key in sig_m.keys():
    ...
    for column in all_data[conns]:
        if all_data[column].std() < 0.05:
            ...
            conns = list(set(conns) - set([column]))
    if session == 'delta':
        p, t, _ = permuted_ols(all_data['{0}{1}'.format(session, iq)].values, 
                               all_data[conns].values, 
                               all_data[['Age', 'Mod', 'Strt.Level', '{0} fd'.format(task)]].values)
    else:
        p, t, _ = permuted_ols(all_data['{0}{1}'.format(iq, session)].values, 
                               all_data[conns].values, 
                               all_data[['Age', 'Mod', 'Strt.Level', '{0} fd'.format(task)]].values)
    triplenetwork_sig = pd.DataFrame(index=['p', 't'], columns=conns)
    triplenetwork_sig.at['p'] = p
    ...
    sig_prewmi_fci_p.fillna(0, inplace=True)
    sig_prewmi_fci_t.fillna(0, inplace=True)
    sig_prewmi_fci_p.to_csv(join(sink_dir, 'm-{0}-{1}{2}_conn_pvals.csv'.format(task, iq, session)))
    sig_prewmi_fci_t.to_csv(join(sink_dir, 'm-{0}-{1}{2}_conn-tvals.csv'.format(task, iq, sessions)))
    q = plot_connectome(sig_prewmi_fci_p.values, coordinates, edge_threshold=1., edge_cmap='Blues')
    q.savefig('/Users/Katie/Dropbox/Projects/physics-retrieval/figures/m-{0}-{1}{2}_conn_sig.png'.format(task, iq, session), dpi=300)
# -
